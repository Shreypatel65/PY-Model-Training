...
client = MongoClient(
    'mongodb://chacha:shrey5510@127.0.0.1:27017/?directConnection=true&serverSelectionTimeoutMS=2000&appName=mongosh+1.10.1')
db = client['demo']
...
def predict_output():
    ...
    # Input_data is in strings, So convert the string to list using eval
    ...
    output = {'pred_columns': pred_columns, 'data_columns': data_columns,
              'model_name': input_data['model_name']}
    output.update(pred_data)  # Add New Prediction Data
    output.update(new_data[0])  # Add Old Feature Data
